...
forecast_date <- "2022-08-04"#args[1]
...
for (geo_val in states){
  ...
  forecast <- run_semi_mech(season_past_matrix_hosp = wave_matrix,season_current_hosp =current_wave ,length_of_current_wave=length_of_current_wave)
  ...
  plot(colMeans(forecast[[1]][,1:((length_of_current_wave + 30))]),type='l',col='red',ylim=c(0,10000))
  lines(current_wave)
...
df_to_submit <- df_to_submit %>% dplyr::select(target,location,forecast_date,target_end_date,quantile,value,type)
write.csv(df_to_submit,file = paste0("/mnt/nfs/scratch1/gcgibson/sub_files7/",as.Date(forecast_date),"-UT-Ankh7.csv"),row.names = F)
